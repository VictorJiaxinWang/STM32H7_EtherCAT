...
//   if (rVal != 0)
 //     return 0;
   ...
   else    //走的是这个分支   	就是把端口结构体里面的数据堆栈中的各个指针赋上实体定义的数据包的地址
   {
     // port->getindex_mutex = mtx_create();
    //  port->tx_mutex = mtx_create();
    //  port->rx_mutex = mtx_create();
     ...
     // port->stack.sock        = &(port->sockhandle);
      ...
      ecx_clear_rxbufstat(&(port->rxbufstat[0])); //将接收数据包的每个buffer的状态都赋上“空”
   ...
   for (i = 0; i < EC_MAXBUF; i++)
   {
		 ec_setupheader(&(port->txbuf[i]));  //将发送数据包中的每个buffer赋上目的地址 源地址 以及以太网帧类型
      port->rxbufstat[i] = EC_BUF_EMPTY;  //这句跟上面的状态复位为“空”重复了
   }
   ec_setupheader(&(port->txbuf2));  //将发送临时缓冲区也装上 目的地址 源地址 和以太网帧类型
	 //？？？ 这个发送临时缓冲区暂时不知道什么意思 接着往下看吧
