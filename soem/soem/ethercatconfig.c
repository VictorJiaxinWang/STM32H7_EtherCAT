...
void ecx_init_context(ecx_contextt *context)
{
   ...
   *(context->slavecount) = 0;
   ...
   ecx_siigetbyte(context, 0, EC_MAXEEPBUF);  //清除从站eeprom缓存 不读任何eeprom
 ...
 * @param[in] usetable     = TRUE when using configtable to init slaves, FALSE otherwise //是否使用默认配置 
 * @return Workcounter of slave discover datagram = number of slaves found  //返回工作计数
 */
int ecx_config_init(ecx_contextt *context, uint8 usetable)
{
   ...
   ecx_init_context(context);   //初始化全局上下文
...
{
   return ecx_config_init(&ecx_context, usetable);  //枚举并且初始化所有从站
}
