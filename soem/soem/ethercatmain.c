 ...
 */
ec_slavet               ec_slave[EC_MAXSLAVE];  //从站结构体数组 所有从站的信息都在这里 下标为0的成员被主站保留 
/** number of slaves found on the network */
int                     ec_slavecount;   //网络上从站的数量
/** slave group structure */
ec_groupt               ec_group[EC_MAXGROUP]; //从站组结果体

/** cache for EEPROM read functions */
static uint8            ec_esibuf[EC_MAXEEPBUF]; //从站信息esi/eeprom中的数据存在这个buffer中
/** bitmap for filled cache buffer bytes */
static uint32           ec_esimap[EC_MAXEEPBITMAP]; //bit位映射
/** current slave for EEPROM cache buffer */
static ec_eringt        ec_elist;
static ec_idxstackT     ec_idxstack;

/** SyncManager Communication Type struct to store data of one slave */
static ec_SMcommtypet   ec_SMcommtype[EC_MAX_MAPT];  //同步管理器类型
/** PDO assign struct to store data of one slave */
static ec_PDOassignt    ec_PDOassign[EC_MAX_MAPT];  //PDO分配结构体
/** PDO description struct to store data of one slave */
static ec_PDOdesct      ec_PDOdesc[EC_MAX_MAPT];  //PDO描述结构

/** buffer for EEPROM SM data */
static ec_eepromSMt     ec_SM;   //EEPROM中保存的同步管理器相关数据 估计就是同步管理器类型
/** buffer for EEPROM FMMU data */
static ec_eepromFMMUt   ec_FMMU;    //EEPROM中保存的FMMU数据
/** Global variable TRUE if error available in error stack */
boolean                 EcatError = FALSE;  //ECAT错误标志

int64                   ec_DCtime;   //分布式时钟时间

ecx_portt               ecx_port;    //端口结构体
ecx_redportt            ecx_redport;  //冗余端口结构体
