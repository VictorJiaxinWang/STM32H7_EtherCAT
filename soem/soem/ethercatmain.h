...
typedef struct
{
   ...
   uint16           state;         //从站状态
   /** AL status code */
   uint16           ALstatuscode;  //AL状态错误码
   /** Configured address */
   uint16           configadr;     //配置地址
   /** Alias address */
   uint16           aliasadr;      //别名地址
   /** Manufacturer from EEprom */
   uint32           eep_man;        //eeprom制造商
   /** ID from EEprom */
   uint32           eep_id;         //eepromid
   /** revision from EEprom */
   uint32           eep_rev;        //eeprom版本号
   /** Interface type */
   uint16           Itype;        //接口类型
   /** Device type */
   uint16           Dtype;     //设备类型
   ...
   uint8            *outputs;    //输出数据指针
   /** startbit in first output byte */
   uint8            Ostartbit;  //输出数据开始的那个bit
   /** input bits */
   uint16           Ibits;      //输入bit数
   ...
   /** SM structure */
   ec_smt           SM[EC_MAXSM];   //同步管理器结构体数组
   /** SM type 0=unused 1=MbxWr 2=MbxRd 3=Outputs 4=Inputs */
   uint8            SMtype[EC_MAXSM];  //同步管理器结构体
   /** FMMU structure */
   ec_fmmut         FMMU[EC_MAXFMMU];   //FMMU结构体
   /** FMMU0 function */
   uint8            FMMU0func;    
   /** FMMU1 function */
   uint8            FMMU1func;
   ...
   uint16           mbx_l;      //写邮箱数据长度
   /** mailbox write offset */
   uint16           mbx_wo;      //写邮箱偏移量
   /** length of read mailbox in bytes */
   uint16           mbx_rl;    //读邮箱数据长度
   /** mailbox read offset */
   uint16           mbx_ro;    //读邮箱数据偏移量
   /** mailbox supported protocols */
   uint16           mbx_proto;    //邮箱支持的协议
...
typedef struct
{
   ...
   uint8            *inputs;   //输如指针 
   /** has DC capabillity */
   boolean          hasdc;    //是否支持dc
   /** next DC slave */
   uint16           DCnext;   //下一个dc从站 
   /** E-bus current */
   int16            Ebuscurrent;
   /** if >0 block use of LRW in processdata */
   uint8            blockLRW;    
   ...
   uint16           nsegments; //段们
   /** 1st input segment */
   uint16           Isegment;  //第一个输入段
   /** Offset in input segment */
   uint16           Ioffset;   //输入段偏移
   /** Expected workcounter outputs */
   uint16           outputsWKC;   //输出的期望工作计数
   /** Expected workcounter inputs */
   uint16           inputsWKC;   //输入的期望工作计数
   /** check slave states */
   boolean          docheckstate;   //是否检查从站状态
   /** IO segmentation list. Datagrams must not break SM in two. */
   uint32           IOsegment[EC_MAXIOSEGMENTS];  //每个组的最大IO段数量
} ec_groupt;
