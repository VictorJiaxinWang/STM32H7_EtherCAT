...
typedef uint8 ec_bufT[EC_BUFSIZE]; //自定义数据类型ec_bufT 本质是一个8位无符号数组 长度1518 实际意义是网络数据包
...
typedef enum
{
   ...
   EC_BUF_ALLOC        = 0x01,       //已分配地址但未填充
   /** Transmitted */
	EC_BUF_TX           = 0x02,      //发送完成
   /** Received, but not consumed */
   EC_BUF_RCVD         = 0x03,  //接收到了 但是还没使用
   /** Cycle completed */
   EC_BUF_COMPLETE     = 0x04   //完成一个发收周期
} ec_bufstate;
